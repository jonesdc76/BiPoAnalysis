...
int BPvT(bool fiducialize = 0, int alpha_type = 0, bool useEsmear = 0, bool P2_style = 0, bool recreate = 0){
  ...
  double hAE = 1.0, lAE = 0.65, hApsd = 0.34, lApsd = 0.18;//alpha
  ...
  //-------------------------------------------
  const int NP = 500;
  TH1D *hE[NP][3], *hdt[NP][3], *hdtbkg[NP], *hZ[NP][3], *hpsd[NP][3];
  TH1D *hZcum[3], *hdtcum[3],*hdtbkgcum, *hdtres, *hEcum[3]; 
  TCanvas *c[NP];
  ...
  for(int i=0;i<NP;++i){
    ...
    hdt[i][0] = new TH1D(Form("hdt%i",i),Form("hdt%i",i), nBINS, t_start, t_end);
    hdt[i][0]->Sumw2();
    hdt[i][0]->SetLineColor(kBlue);
    hdt[i][1] = new TH1D(Form("hdtfar%i",i),Form("hdtfar%i",i), nBINS, t_start, t_end);
    hdt[i][1]->Sumw2();
  ...
  int nbins = 100;
  hdtcum[0] = new TH1D("hdtcum","hdtcum",  nbins, t_start, t_end);
  hdtcum[0]->Sumw2();
  hdtcum[0]->SetLineColor(kBlue);
  hdtcum[1] = new TH1D("hdtcumfar","hdtcumfar",  nbins, t_start, t_end);
  hdtcum[1]->Sumw2();
  hdtcum[1]->SetLineColor(kMagenta);
  hdtbkgcum = new TH1D(Form("hdtbkgcum"),Form("hdtbkgcum"), 100, l_t, h_t);
  hdtbkgcum->Sumw2();
  hdtres = new TH1D("hdtres","hdtres",  nbins, t_start, t_end);
  hdtres->Sumw2();
  hdtres->SetLineColor(kGreen);
  hEcum[0] = new TH1D("hEcum","hEcum", nBINS, lAE, hAE);
  hEcum[0]->Sumw2();
  hEcum[0]->SetLineColor(kBlue);
  hEcum[1] = new TH1D("hEcumfar","hEcumfar", nBINS, lAE, hAE);
  hEcum[1]->Sumw2();
  hEcum[1]->SetLineColor(kMagenta);
  hZcum[0] = new TH1D("hZcum","hZcum", nbins, -1000, 1000);
  hZcum[0]->Sumw2();
  hZcum[0]->SetLineColor(kBlue);
  hZcum[1] = new TH1D("hZcumfar","hZcumfar", nbins, -1000, 1000);
  hZcum[1]->Sumw2();
  ...
  double time_eff = exp(-t_start/(alpha_type==1 ? 0.00043 : tauBiPo));
  time_eff -= exp(-t_end/(alpha_type==1 ? 0.00043 : tauBiPo));
  int nr = 0;
  ...
  PROCESS_HISTOS:
	...
	hdtbkg[p]->Scale(1/hdtbkg[p]->GetBinWidth(1)/tlive[p]);
	...
	if(alpha_type==1){
	  ...
	  hdtbkg[p]->Fit(fdecay,"B");
	  gR2->SetPoint(p,t[p], fdecay->GetParameter(2)*tau);
	  gR2->SetPointError(p,0, fdecay->GetParError(2)*tau);
	  fdecay->ReleaseParameter(0);
	  fdecay->SetParameter(0, hdt[p][2]->GetMaximum()*5);
	...
	gAcc->SetPoint(p,t[p],fpol0->GetParameter(0)*(t_end-t_start)*1000);
	TFitResultPtr ftr = hdt[p][0]->Fit(fdecay, "SRB");
	...
	double err;
	double r = hE[p][2]->IntegralAndError(1,hE[p][2]->GetNbinsX(), err);
	r /= eff[p] * time_eff;
	err /= eff[p] * time_eff;
	gR->SetPoint(p,t[p],r);
    ...
    for(int j=0;j<bp->mult_far;++j){
      if( (bp->fmult_clust->at(j) != bp->fmult_clust_ioni->at(j)) )
      	continue;//throw out clusters with recoils mixed in
      if(bp->fEtot->at(j) < lowBE || bp->fEtot->at(j) > highBE)continue;
      if(bp->fPSD->at(j) < lPpsd || bp->fPSD->at(j) > hPpsd)continue;
      if(!(fabs(bp->fz->at(j)) < 1000)) continue;
      if(fiducialize && (isET(bp->fseg->at(j)) || fabs(bp->fz->at(j))>fidZ))continue;
  ...
  if(alpha_type==1){
    chl->cd(2);
    gR2->SetTitle(Form("#chi^{2}/NDF for Half-life Fit vs Time"));
    gR2->GetXaxis()->SetTimeDisplay(1);
    gR2->GetXaxis()->SetTimeFormat("%m/%d");
    gR2->GetXaxis()->SetTitle("Date in 2018");
    gR2->SetTitle(Form("Rate of Po214 vs Time"));
    gR2->GetYaxis()->SetTitle(Form("Rate (Hz)"));
    gR2->Fit("fpol0");
    rate214 = fpol0->GetParameter(0);
    rate214err = fpol0->GetParError(0);
    rate214prob = fpol0->GetProb();
    AddShade(gR2);
  }
  chl->cd(3);
  ...
  if(alpha_type == 0){
    fdecay->SetParameters(hdtcum[2]->GetMaximum(), 0.00043);
    fdecay->FixParameter(2, fpol0->GetParameter(0));
  }
  ...
  TCanvas *cEcum = new TCanvas("cEcum","cEcum",600,0,700,500);
  hEcum[0]->Draw();
  hEcum[1]->Draw("same");
  hEcum[2] = (TH1D*)hEcum[0]->Clone("hEcum");
  hEcum[2]->SetLineColor(kRed);
  hEcum[2]->Add(hEcum[1],-1);
  hEcum[2]->Draw("sames");
